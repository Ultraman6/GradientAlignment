...
def partition_noniid(dataset: torch.utils.data.Dataset, workers: int, percentage_iid: float, use_two_splits):
    ...
    if use_two_splits == False:
        return [torch.utils.data.Subset(dataset, torch.cat((iid_indices[i], structured_noniiid_indices[i]))) for i in
                range(workers)]
    else:
        return [[torch.utils.data.Subset(dataset, iid_indices[i]),
                 torch.utils.data.Subset(dataset, structured_noniiid_indices[i])] for i in range(workers)]
...
def split_data(task: str, workers: int, percentage_iid: float, use_two_splits: bool):
    if task == "MNIST":
        dataset = datasets.MNIST(root=Arguments.data_home, train=True, download=True,
                                 transform=MNISTTransform.train_transform())
    elif task == "EMNIST":
        dataset = datasets.EMNIST(root=Arguments.data_home, split="balanced", train=True, download=True,
                                  transform=EMNISTTransform.train_transform())
    elif task == "CIFAR100":
...
def get_validation_data(task: str):
    if task == "MNIST":
        dataset = datasets.MNIST(root=Arguments.data_home, train=False, download=True,
                                 transform=MNISTTransform.train_transform())
    elif task == "EMNIST":
        dataset = datasets.EMNIST(root=Arguments.data_home, split="balanced", train=False, download=True,
                                  transform=EMNISTTransform.train_transform())
    elif task == "CIFAR100":
        dataset = datasets.CIFAR100(root=Arguments.data_home, train=False, download=True,
                                    transform=CIFAR100Transform.test_transform())
    elif Arguments.task == "shakespeare":
        return get_shakespeare_ds(train=False)
    elif Arguments.task == "sent140":
        return get_sent140_ds(train=False)
    else:  # task == "CIFAR10"
        dataset = datasets.CIFAR10(root=Arguments.data_home, train=False, download=True,
                                   transform=CIFAR10Transform.test_transform())
    return dataset
